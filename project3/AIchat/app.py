...
@app.route('/chat/<uid>')
def chat(uid):
    session['uid'] = uid #세션에 저장해서 app전체에서 사용가능
    session['room'] = uid #세션에 저장해서 app전체에서 사용가능
...
def text(data):
    ...
    answer_txt = answer(msg)
    emit('message', {'msg':f'[인공지능] {answer_txt}'}, room=room)
    speak(answer_txt)
